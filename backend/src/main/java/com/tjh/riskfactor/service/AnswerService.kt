...
class AnswerService: IDBService<Answer> {
    ...
    fun importExcel(taskId: Int, userId: Int, istream: InputStream) {
        ...
        ExcelReader(istream).use { reader ->
            val answers = reader.cells().map { dataCell ->
                ...
                val question = layout[title]?.second?.get(h3)
                AnswerEntry().apply {
                    this.answer = answer; this.question = question
                    this.value = question?.let { formatValue(cell, it) }
                }
            }
    ...
    override val entityName = "answer"
