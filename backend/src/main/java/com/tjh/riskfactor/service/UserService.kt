...
class UserService: IDBService<User> {
    ...
    @Autowired private lateinit var repo: UserRepository
    @Autowired private lateinit var encoder: PasswordEncoder

    fun has(username: String) = repo.existsByUsername(username)

    fun find(username: String): User? = repo.findByUsername(username).orElse(null)
     ...
     * @return 用户组名
     */
    fun groupNameOf(uid: Int): String? = repo.findGroupName(uid).orElse(null)
     ...
     * @return 管理的组id
     */
    fun managedGroupId(uid: Int): Int? = repo.findManagedGroupId(uid).orElse(null)
